...
<script setup lang="ts">
   ...
   const getUneRecette = async () => {
     ...
     const idMeal = window.location.pathname.split('Detail/')[1];
     if (idMeal !== undefined) {
       url = url + idMeal;
     }
     ...
     fetch(url)
       ...
       .then((data) => {
         console.log("(Detail.vue) Recette -> ");
         console.log(data);
         ...
         console.log("(Detail.vue) Nombre d'ingredients -> " + (ingredients.value ? ingredients.value.length : 0));
         loading.dismiss();
       });
   };
   
   onMounted(() => {
     console.log('(Detail.vue) did enter');
     getUneRecette();
   });
</script>
